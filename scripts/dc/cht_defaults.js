...
function referralSourceColours(inColour) {
	// Base Colours (originally designed around diagnosis codes)
	// http://colorbrewer2.org/?type=sequential&scheme=BuPu&n=9#type=qualitative&scheme=Paired&n=12
	const mapColours = new Map([
		[1, "#1f77b4"], // Cons to Cons
		[2, "#ff7f0e"], // GP
		[20, "#ff7f0e"], // GO
		[3, "#2ca02c"], // Other
		[0, "#808080"], // unknown, grey
	...
	// Just need the first character of the inColour eg. 1, 2 or 3
	const arr = inColour;

	// Base Colour
	colour = startColour = endColour = d3.hcl(
		mapColours.get(+arr.substring(0, 1))
	);
	...
	if (arr.length === 1) {
		...
		colours.range([startColour, endColour]);
		return colours(arr.slice(-2)); //  the last item in the array
	}
}
