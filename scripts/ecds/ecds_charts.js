...
const heatmapLegendCells = heatmapLegend
	.append('g')
	.attr('transform', 'translate(' + 15 + ',' + 5 + ')'); // (height / 2)

const heatmapLegendLabels = heatmapLegend
	.append('g')
...
function heatKey(noCells) {
	...
	const t = d3
		.transition('test')
		.duration(500) // 750 too slow and results in transition getting stuck
		.ease(d3.easeBounce);
			...
			(
				...
				exit.call((exit) =>
					exit
						...
						.delay(function(d, i) {
							// a different delay for each rect
							return i * 50; // (noCells - i) * 50 to reverse
						})
						.attr('y', 30)
						.remove()
				)
