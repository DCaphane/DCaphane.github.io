...
Papa.parse('Data/ecds_ref_tables/ref_diagnosis.csv', {
	...
	complete: (d) => {
		diagnosisRefObj = d.data.reduce(
			(obj, item) => (
				(obj[item.SNOMED_Code] = [item.Sort1, item.Sort2, item.Sort3]),
				obj
			),
			{}
		);
...
Papa.parse('Data/ecds_ref_tables/ref_acuity.csv', {
	...
	complete: (d) => {
		acuityRefObj = d.data.reduce(
			(obj, item) => (
				(obj[item.SNOMED_Code] = item.ECDS_Description), obj
			),
			{}
		);
...
Papa.parse('Data/ecds_ref_tables/ref_attd_source.csv', {
	...
	complete: (d) => {
		attdSourceRefObj = d.data.reduce(
			(obj, item) => (
				(obj[item.SNOMED_Code] = [item.Sort1, item.Sort2]), obj
			),
			{}
		);
	...
	complete: (d) => {
        ...
        d[sqlCols.practice] = +practiceObj[d[sqlCols.practice]][0];
			...
			if (diagnosisRefObj[+d[sqlCols.diagnosis]] !== undefined) {
				...
				d.DiagnosisMainGroup =
					diagnosisRefObj[+d[sqlCols.diagnosis]][0];
			} else {
		...
		seriesDaily
			...
			.x(
				d3
					.scaleTime()
        .domain([minDateTS - (1000 * 60 * 60 * 24), maxDateTS + (1000 * 60 * 60 * 24)]) // add & remove one day in ms
					.range([
...
*/
		...
		const dimTimeofDay = cf.dimension(function(d) {
			return [d[sqlCols.hour], d[sqlCols.weekday]]; // hour, weekday number
		});
		...
		const dimDuration = cf.dimension(function(d) {
				return d3.min([d[sqlCols.duration], maxDuration]); // cut off at eg. 8 hours
			}),
		...
		let groupArrayDuration = dimWeekdayNo.group().reduce(
			function(p, v) {
				p.push(d3.min([v[sqlCols.duration], maxDuration]));
				return p;
			},
			function(p, v) {
				p.splice(
					p.indexOf(d3.min([v[sqlCols.duration], maxDuration])),
					1
				);
				return p;
			},
...
function sunburstColours(inColour) {
	...
	if (arr.length === 1) {
		...
		for (let i = 2; i <= arr.length; i++) {
			(startColour = startColour.darker()),
				(endColour = endColour.brighter());
		}
