/*
Leaflet Conditional Layer
    Loads a given number of markers visible in the frame
    Inspired on the work of Ishmael Smyrnow

    https://github.com/Eclipse1979/leaflet-conditionalLayer
    https://raw.githubusercontent.com/Eclipse1979/leaflet-conditionalLayer/master/dist/leaflet.conditionalLayer.js
*/
...
L.ConditionalMarkers = L.FeatureGroup.extend({
  ...
  _addMarkers: function () {
    ...
    const markers = this._getMarkersInViewport(this._map);
  ...
  _cleanupMarkers: function () {
    ...
    this._layer.eachLayer(function (marker) {
      if (
        ...
        (!marker._popup || !marker._popup._container)
      ) {
...
L.conditionalMarkers = function (markers, options) {
  const layer = new L.ConditionalMarkers(null, options);
  let i;
  for (i in markers) {
