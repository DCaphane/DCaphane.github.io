/*
For d3.csv, include column headings
For papaparse, remove data headings {no longer use}

To Consider:
	Firsts only?
	Chart to show Practice population and att'd (currently in dropdown) - nice to do but not necessary
...
declare @startPeriod date = '2018-07-01';
...
select -- cast(arrival_date as datetime) + cast(arrival_time as datetime) as 'Arrival_DateTime'
    ...
    , case
	when nullif(age_at_admission, '999') is null then 8
	when age_at_admission > 84 then (85 / 5)
	else age_at_admission / 5 end as 'AgeBand'
